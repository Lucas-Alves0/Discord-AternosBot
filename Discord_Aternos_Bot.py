...
user = {
  'username': 'SrLks7162', 'password': 'L6070al#'
}
...
dctoken = {
  'TOKEN':'MTMxMzY0NTM4NzkxMDc0NjEyMg.GSY92J.argtbVI8M3VvMjztAbojaj5ZcP1ka4Brc3gyqk'
}
@dcbot.event
async def on_ready():
...
async def iniciar(ctx):
  ...
  try:
    ...
    myserv.fetch()
    await ctx.send(f'O servidor está iniciando, aguarde\n\nStatus de abertura do servidor: {myserv.status}')
...
async def parar(ctx):
  ...
  while myserv.status != 'offline':
    myserv.fetch()
    await ctx.send(f'O servidor está sendo fechado, aguarde\n\nStatus de fechamento do servidor: {myserv.status}')
  else:
    sleep(15)
    myserv.fetch()
